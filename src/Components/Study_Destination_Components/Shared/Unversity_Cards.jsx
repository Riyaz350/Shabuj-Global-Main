...
export default function Unversity_Cards({ country, apiRoute , routeLabel}) {
    const [selectedUniversity, setSelectedUniversity] = useState(null);
    const [universities, setUniversities] = useState([]);
  
    useEffect(() => {
        fetch(`./universities_json/${apiRoute}.json`)
            ...
            {universities.map((university, index) => (
              ...
              >
                <img
                  src={university.logo}
                  alt={university.name}
                  className="w-[100px] h-24 object-contain mb-4 lg:px-4 md:px-4 px-0"
                />
              </div>
            ))}
          </div>
          ...
          >
            {selectedUniversity && (
              <div className="p-4 bg-[EFF6FF] rounded-lg">
                ...
                <Link to={`/${routeLabel}/${selectedUniversity.id}`}>
