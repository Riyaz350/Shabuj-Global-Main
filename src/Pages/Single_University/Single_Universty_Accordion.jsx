...
const Single_Universty_Accordion = () => {
  ...
  return (
    <div className="w-full h-screen rounded-lg px-5 py-10">
      {dataArr.map((PerAccordion, idx) => (
        <div key={idx} className="my-3">
          ...
          >
            <span className="pl-5 py-3 text-lg md:text-2xl font-bold text-[#00399F]">{PerAccordion.title}</span>
            <span className={`rounded-full text-lg md:text-xl px-8 py-4 ${isOpen === idx ? "bg-[#00399F]" : "bg-[#e1e1e1]"}`}>
              <LuTriangle
          ...
          <div
            className={`grid overflow-hidden transition-all duration-300 ease-in-out ${
              isOpen === idx ? "grid-rows-[1fr] pb-1 pt-3 opacity-100" : "grid-rows-[0fr] opacity-0"
            }`}
          >
